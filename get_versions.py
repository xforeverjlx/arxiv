...
json_file = "test_major_cats_full.json"
output_file_prefix = "test_major_"
...
for chunk in pd.read_json(json_file, lines=True, chunksize=chunksize):
  prime_freqs = prime_freqs + Counter(dict(chunk.loc[chunk['version']!=1, 'prime_category'].value_counts()))
  major_freqs = major_freqs + Counter(dict(chunk.loc[chunk['version']!=1, 'major_category'].value_counts()))
  wrds = chunk.loc[chunk['version']!=1, 'abs_categories'].str.split()
...
for t in ['prime', 'major', 'abs_cat']:
  with open("versions/"+output_file_prefix+t+".txt", "w") as f:
